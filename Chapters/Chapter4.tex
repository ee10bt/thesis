...
\begin{table}[h]
\centering
\caption{Parameters used to define \citet{Oganov2000}'s \mgsios perovskite potential, including my fit Fe-O values.} %\vspace{2mm}
...
At this point, we have all the pieces in place to qualitatively describe how the equation for modelling a solid solution's thermal conductivity (\ref{eq.ohta7}) works and why. The first part is to scale a linearly interpolate the conductivity of two endmembers, at a given temperature, to the composition of interest. This ($\kappa_{\mathrm{V}}$) will most often be an overestimate of the true value, the conductivity is more complicated than a weighted average of the endmembers. 

The atomic disorder serves to reduce conductivity, a function of both temperature and composition. The more the solid solution is varied (i.e. closer to a 50/50 mix), the larger the effect of impurities on conductivity

ARCTAN!!! of relative magnitudes
