...
%-------------------
\subsubsection{$\omega_{0}$ in depth} 
%-------------------
...
$A$ and $B$ are constants, the latter directly proportional to temperature (T). The two above terms can be equated via Equation \ref{eq.Klemens11}, showing $\omega_{0}$ is similarily temperature-dependent
%
...
TALK ABOUT DEBYE FREQUENCY, IS IT A CONSTANT? DEPENDENT ON SOUND VELOCITY, THEREFORE SMALLER WITH TEMPERATURE, A FUNCTION OF TEMPERATURE? 

DEBYE TEMPERATURE DEPENDS ON DEBYE FREQUENCY, WHICH DEPENDS ON SOUND VELOCITY IN MATERIAL, WHICH DEPENDS ON TEMPERATURE, SO DEBYE TEMPERATURE DEPENDS ON TEMPERATURE???

\pagebreak

%-------------------
\subsubsection{How do impurities affect phonon relaxation time?} 
%-------------------
...
%-------------------
\subsubsection{What affects the magnitude of impurity scattering?} 
%-------------------
...
As the temperature increases, so too does the left-hand side of Equation \ref{eq.SS2015SM.11}. As discussed earlier, this reduces the effect of scattering caused by impurities, which will be relevant at CMB conditions. 
