...
That factors that affect the severity of impurity scattering are temperature, the mass difference between the impurity and what it replaced, and the concentration of said replacements. The ratio of the phonon frequencies in Eq. \ref{eq.SS2015SM.6} can be expressed \citep[][Eq. S11]{Stackhouse2015} as
%
\begin{equation}
...
\end{equation}
%
As temperature increases (above the Debye temperature), conductivity decreases because of Umklapp processes. The significance (SHOWN IN A LATER FIGURE) of point defect scattering decreases as phonon-phonon scattering becomes more prominent. Therefore the relative conductivity decrease due to impurities is inversely proportional with temperature.

IS DEBYE FREQUENCY A CONSTANT? DEPENDENT ON SOUND VELOCITY, THEREFORE SMALLER WITH TEMPERATURE, A FUNCTION OF TEMPERATURE? DEBYE TEMPERATURE DEPENDS ON DEBYE FREQUENCY, WHICH DEPENDS ON SOUND VELOCITY IN MATERIAL, WHICH DEPENDS ON TEMPERATURE, SO DEBYE TEMPERATURE DEPENDS ON TEMPERATURE???
