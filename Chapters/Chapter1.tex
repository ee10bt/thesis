...
!!! ADD NICE FIGURE - Cv driving regime, MFP driving regime, saturation

The phonon MFP increases with the volumetric heat capacity, until the saturation and convergence of this parameter at a material's Debye temperature. Lattice conductivity is proportional to the MFP (Eq.\ref{eq.cvvl}), so also increases with temperature up to the Debye limit. Phonon-phonon scattering occurs as long as there are phonons, the effect of which increases with temperature past the Debye temperature. Conductivity change is inversely proporitional to temperature at this point, decreasing, and eventually saturating, to a minimum value as the MFP reaches its minimum (on the order of atomic spacing). Bridgmanite is above its Debye temperature for all lower mantle conditions I will consider. I can safely assume its conductivity will decrease with rising temperature.
