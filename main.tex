...
\usepackage{multirow} % Makes the tables look handsome 

\usepackage{rotating} % sidewaystable and ting

%\usepackage{gensymb} % gives degree symbol for angles, \degree
...
\newcommand{\pv}{perovskite}
\newcommand{\pvs}{perovskite }
\newcommand{\ppv}{post-perovskite}
\newcommand{\ppvs}{post-perovskite }
\newcommand{\bdg}{bridgmanite}
\newcommand{\bdgs}{bridgmanite }
...
\newcommand{\ddd}{D$^{\prime\prime}$}
\newcommand{\ddds}{D$^{\prime\prime}$ }

\newcommand{\ai}{\textit{ab initio}}
\newcommand{\ais}{\textit{ab initio} }

\newcommand{\coc}{$C \left (1-C \right)$}
\newcommand{\cocs}{$C \left (1-C \right)$ }

\newcommand{\degsym}{^{\circ}} % i should probably use ``\usepackage{gensymb}'' and ``\degree'', but this old template is rogering me over
...
\thesistitle{Simulating the lattice thermal conductivity of iron-bearing bridgmanite with implications on core-mantle boundary heat flux} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
\vspace*{0.02\textheight} %\vspace*{0.02\textheight}
